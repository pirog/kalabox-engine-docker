...
module.exports = function(kbox) {
  ...
  if (util.needsAdminCommands()) {
    kbox.install.registerStep(function(step) {
      ...
      step.all.linux = function(state) {
        ...
        // Add the VB install command if we need it
        if (util.needsVB()) {

          // Get the VB pkg location
          var nix = kbox.install.linuxOsInfo.get();
          var pkgDir = kbox.util.disk.getTempDir();
          var vb = meta.PROVIDER_DOWNLOAD_URL.linux.vb;
          var pkgName = path.basename(vb[nix.ID][nix.VERSION_ID]);
          var pkg = path.join(pkgDir, pkgName);

          // Add VB install as an admin command
      ...
      step.all.win32 = function(state) {
        ...
        if (util.needsB2D()) {
          ...
          var cmd = kbox.install.cmd.buildInstallCmd(pkg, infFile);
          state.adminCommands.push(cmd);
        }
  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all = function(state, done) {
      ...
      // Calculate a reasonable size for the VM based on free space
      // if KALABOX_VMSIZE env is set then use that instead
      // Units should be in MB
      var options = {};
      if (process.env.KALABOX_VMSIZE !== undefined) {
        var diskSize = process.env.KALABOX_VMSIZE;
        if (diskSize > 2000) {
