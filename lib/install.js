  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all = function(state, done) {
      ...
      return Promise.try(function() {
        // Only run if not prepackaged.
        if (!kbox.core.deps.get('prepackaged')) {
          // Find out if profile exits.
          var dst = path.join(state.config.sysProviderRoot, 'profile');
          // Only continue if profile doesn't exists.
          if (!fs.existsSync(dst)) {
            // Build src filepath.
            var src = path.join(
              kbox.util.disk.getTempDir(),
              path.basename(meta.PROVIDER_PROFILE_URL)
            );
            // Make sure sys provider directory exists.
            return Promise.fromNode(function(cb) {
              mkdirp(state.config.sysProviderRoot, cb);
            })
            // Move file.
            .then(function() {
              return Promise.fromNode(function(cb) {
      ...
      step.all = function(state) {
        if (!kbox.core.deps.get('prepackaged')) {
          ...
          state.downloads.push(
            meta.PROVIDER_DOWNLOAD_URL[process.platform].b2d
          );
