...
module.exports = function(kbox) {
  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all.darwin = function(state, done) {
      sysProfiler.isAppInstalled(BOOT2DOCKER, function(err, isInstalled) {
        if (err) {
          ...
          state.log.debug(BOOT2DOCKER + ' is installed?: ' + isInstalled);
          done();
        }
      });
    };
    ...
    step.all.win32 = function(state, done) {
      ...
      state.log.debug('Boot2Docker ' + msg + ' installed.');
      done();
    };
  });
  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all = function(state, done) {
      ...
      fs.exists(state.boot2dockerProfileFilepath, function(exists) {
        ...
        state.log.debug('Boot2docker profile set?: ' + exists);
        done();
      });
    };
  });
  ...
  kbox.install.registerStep(function(step) {
    ...
    step.all.linux = function(state, done) {
      ...
      shell.exec(cmd, function(err, data) {
        ...
        state.log.debug('VBoxManage installed? ' + state.vbIsInstalled);
        done();
      });
    };
  });
